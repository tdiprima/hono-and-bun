...
var xhr_default = isXHRAdapterSupported && function(config) {
  return new Promise(function dispatchXhrRequest(resolve, reject) {
    ...
    request.ontimeout = function handleTimeout() {
      ...
      reject(new AxiosError_default(timeoutErrorMessage, transitional3.clarifyTimeoutError ? AxiosError_default.ETIMEDOUT : AxiosError_default.ECONNABORTED, _config, request));
      request = null;
    };
...
async function fetchUser(user) {
  const res = await axios_default.get(`https://api.github.com/users/${user}`);
  return res.data;
}
var githubAPI_default = fetchUser;

// src/index.ts
(async () => {
  const userData = await githubAPI_default("tdiprima");
